...
void StageARunAction::BeginOfRunAction(const G4Run *run)
{
    ...
    fSummaryCsvPath = BuildNextSummaryCsvPath();
...
void StageARunAction::EnsureSummaryCsvReady()
{
    ...
    if (!FileExists(fSummaryCsvPath))
    {
        ...
        // metadata lines
        fout << "# placement_file," << CsvQuote(placementFile) << "\n";
        fout << "# placement_basename," << CsvQuote(placementBaseName) << "\n";
...
std::string StageARunAction::BuildNextSummaryCsvPath() const
{
    const std::string dir = BuildStageARatioDirectory();

    for (int idx = 1;; ++idx)
    {
        std::ostringstream oss;
        oss << dir << "/neutron_transport_summary_" << idx << ".csv";
